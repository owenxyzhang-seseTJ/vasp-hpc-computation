...
semi-automated workflows on an HPC cluster accessed over a direct SSH
connection:

```text
workstation -> HPC login node
```
...
- Upload local calculation folders to the HPC over SSH.
...
|----------|---------|---------|
| `HPC_HOST` | `hpc-login` | SSH alias (in `~/.ssh/config`) reaching the HPC login node |
...
The helpers assume one SSH alias: `hpc-login` (override via `HPC_HOST`). Example
config — replace the placeholders with your real details.
...
> If your SSH alias differs, either rename it in your config or override
> `HPC_HOST`.
...
理论计算任务转化为可审计、半自动化的工作流，运行在通过直接 SSH 连接访问的
HPC 集群上：
...
| 变量 | 默认值 | 含义 |
|----------|---------|---------|
| `HPC_HOST` | `hpc-login` | `~/.ssh/config` 中通往 HPC 登录节点的 SSH 别名 |
...
辅助命令假定一个 SSH 别名：`hpc-login`（可通过 `HPC_HOST` 覆盖）。以下为
示例配置 —— 请将占位符替换为你的真实信息。
...
> 若你的 SSH 别名不同，可在配置中重命名，或覆盖 `HPC_HOST`。
